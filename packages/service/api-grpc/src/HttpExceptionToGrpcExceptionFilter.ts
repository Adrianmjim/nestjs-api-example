...
@Catch(HttpException)
export class HttpExceptionToGrpcExceptionFilter implements ExceptionFilter {
  private readonly httpExceptionToGrpcExceptionMap: Record<number, number> = {
  ...
  public catch(exception: HttpException): Observable<never> {
    const httpStatus: number = exception.getStatus();
    const httpResponse: { details?: unknown } = exception.getResponse() as {
      details?: unknown;
    };

    return throwError(() => ({
      code: this.httpExceptionToGrpcExceptionMap[httpStatus] ?? status.UNKNOWN,
      details: Array.isArray(httpResponse.details) ? httpResponse.details : undefined,
