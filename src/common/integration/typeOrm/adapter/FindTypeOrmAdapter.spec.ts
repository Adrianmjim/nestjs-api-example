...
describe(FindTypeOrmAdapter.name, () => {
  ...
  beforeAll(() => {
    queryBuilderMock = Object.assign(Object.create(SelectQueryBuilder.prototype), {
      getMany: jest.fn(),
      getOne: jest.fn(),
      select: jest.fn().mockReturnThis(),
    } as Partial<jest.Mocked<SelectQueryBuilder<ModelTest>>> as jest.Mocked<SelectQueryBuilder<ModelTest>>);
  ...
  describe('.find()', () => {
    describe('when called', () => {
      ...
      it('should call findQueryToFindQueryTypeOrmConverter.convert()', () => {
        expect(findQueryToFindQueryTypeOrmConverterMock.convert).toHaveBeenCalledTimes(1);
        expect(findQueryToFindQueryTypeOrmConverterMock.convert).toHaveBeenCalledWith(
          queryTestFixture,
          queryBuilderMock,
        );
      });
      ...
      it('should call modelDbToModelConverter.convert()', () => {
        ...
        expect(modelDbToModelConverter.convert).toHaveBeenCalledWith(modelTestFixture);
      });
  ...
  describe('.findOne()', () => {
    describe('when called and findQueryToFindQueryTypeOrmConverter returns FindConditions<TModelDb> and repository.findOne() returns undefined', () => {
      ...
      it('should call repository.findOne()', () => {
        ...
        expect(repositoryMock.findOne).toHaveBeenCalledWith(queryTypeOrmFixture);
      });
