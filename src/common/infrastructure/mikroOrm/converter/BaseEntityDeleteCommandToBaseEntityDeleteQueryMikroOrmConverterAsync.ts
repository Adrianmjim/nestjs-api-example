...
> implements ConverterAsync<TInput, TOutput>
{
  public async convert(input: TInput): Promise<TOutput> {
    ...
    const output: TOutput = await this.convertToEntityDeleteQueryMikroOrm(input, baseEntityDeleteQueryMikroOrm);
  ...
  protected abstract convertToEntityDeleteQueryMikroOrm(
    input: TInput,
